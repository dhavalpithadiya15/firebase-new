...
class Helper {
  ...
  static Future<void> addUserDataToFireStore(UserCredential userDetail) async {
    ...
    var userExistInFireStore = querySnapshot.docs
        .map((e) => e.data())
        .toList()
        .where((element) => element['uid'] == userDetail.user!.uid)
        .toList();
  ...
  static void getIncomingAction() {
    FlutterPinnedShortcut flutterPinnedShortcut = FlutterPinnedShortcut();
    flutterPinnedShortcut.getLaunchAction((action) {
      switch (action) {
        case "demo":
      }
    });
