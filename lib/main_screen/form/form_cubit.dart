...
class MyFormCubit extends Cubit<MyFormState> {
  ...
  Future<void> editData(String documentId) async {
    print("===$documentId");
    QuerySnapshot<Map<String, dynamic>> querySnapShot = await FirebaseFirestore.instance.collection("form_data").where("documentId", isEqualTo: documentId,).get();
    querySnapShot.docs.map((e) => e.data()).toList().forEach((element) {
      state.nameController.text = element['name'];
      state.numberController.text = element['phoneNumber'];
      state.emailController.text = element['email'];
      ...
      emit(state.copyWith(selectedGender: element['gender'], checkListItems: temList, selectedCountry: element['address']['country'], selectedState: element['address']['state'], selectedCity: element['address']['city']),
      );
    });
  }
